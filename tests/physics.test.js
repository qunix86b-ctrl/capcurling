...
test('weak shot retires before qualifying line',()=>{const c=cap({vy:-300});simulate([c]);assert.equal(c.reason,'기준선 미달');});
...
test('cap below the line stays in play until the final result',()=>{const c=cap({vy:-300});simulate([c],false);assert.equal(c.retired,false);assert.ok(c.y>C.line);settle([c],true);assert.equal(c.reason,'기준선 미달');});
